...
public class SAXChunkParser : Object {
    ...
    private   ParserCtxt pctx;
    private   bool       is_first_chunk;
    ...
    public void parse_chunk (string buffer) {
        if (is_first_chunk) {
            pctx = new ParserCtxt.create_push (
                &handler, this, (char []) buffer.data, buffer.length, null);
            is_first_chunk = false;
        }
        else {
            pctx.parse_chunk ((char []) buffer.data, buffer.length, false);
        }
    }
